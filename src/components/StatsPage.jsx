...
function ChartBlock({ title, meta, data, dataKey, color, maxY }) {
  ...
  // ارتفاع متغيّر: نعطي كل طالب مساحة كافية لقراءة اسمه.
  const chartHeight = Math.max(180, sorted.length * 38 + 40)
      ...
      {!hasData ? (
        ...
        <div className="chart-box" style={{ height: chartHeight }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#e3eef2" horizontal={false} />
              <XAxis
                type="number"
                allowDecimals={false}
              ...
              <YAxis
                ...
                width={92}
                interval={0}
                tick={{ fontFamily: 'IBM Plex Sans Arabic', fontSize: 12, fill: '#163a4f' }}
              />
              <Tooltip
                ...
                labelFormatter={(l) => l}
              />
              <Bar dataKey={dataKey} radius={[0, 6, 6, 0]} maxBarSize={26} label={{ position: 'left', fill: '#163a4f', fontFamily: 'IBM Plex Sans Arabic', fontSize: 12 }}>
